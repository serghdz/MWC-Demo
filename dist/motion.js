...
(()=>{const canvas=document.querySelector('#star-canvas'),hero=document.querySelector('.hero');if(!canvas)return;const ctx=canvas.getContext('2d');if(!ctx)return;let w=0,h=0,t=0,last=0,visible=true;const stars=Array.from({length:145},(_,i)=>({x:((Math.sin(i*87.3+1)*43758.5)%1+1)%1,y:((Math.sin(i*19.7+4)*43758.5)%1+1)%1,r:.4+(i%5)*.19,phase:i*2.41,speed:.02+(i%4)*.007}));function resize(){w=hero.clientWidth;h=hero.clientHeight;const d=Math.min(devicePixelRatio,1.5);canvas.width=w*d;canvas.height=h*d;ctx.setTransform(d,0,0,d,0,0)}new ResizeObserver(resize).observe(hero);new IntersectionObserver(e=>visible=e[0].isIntersecting).observe(hero);function frame(now){requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.05);last=now;if(!visible||document.hidden)return;if(!window.missionMotion.paused)t+=dt;ctx.clearRect(0,0,w,h);for(const s of stars){const x=(s.x*w+Math.sin(t*s.speed+s.phase)*9+w)%w,y=(s.y*h-t*(.6+s.speed*12)+h*10)%h;ctx.globalAlpha=.10+(.5+.5*Math.sin(t*.45+s.phase))*.27;ctx.fillStyle=s.phase%2>1?'#a8c6db':'#bce4da';ctx.beginPath();ctx.arc(x,y,s.r,0,Math.PI*2);ctx.fill()}ctx.globalAlpha=1}resize();requestAnimationFrame(frame)})();
