...
try{await startHero()}catch(error){console.error('The 3D hero could not load; showing the complete emblem.',error);canvas.hidden=true;host.classList.remove('loaded');window.missionHeroReady=false;window.dispatchEvent(new Event('hero-ready'))}
async function startHero(){
 ...
 const [gltf,land,font]=await Promise.all([new GLTFLoader().loadAsync('./assets/cross.glb'),fetch('./assets/land.json').then(r=>{if(!r.ok)throw Error('Map unavailable');return r.json()}),new FontLoader().loadAsync('./assets/helvetiker_regular.typeface.json')]);
 ...
 const orbit=new THREE.Group();root.add(orbit);const phrase='MISSION WORLD CHURCH   MISSION WORLD CHURCH   ';const letters=[],glyphCache=new Map();
 ...
 const fixedFacing=new THREE.Quaternion();
 ...
 function pose(dt){
  ...
  root.updateMatrixWorld(true);root.getWorldQuaternion(fixedFacing).invert().multiply(camera.quaternion);
  letters.forEach((letter,i)=>{if(!letter)return;const a=Math.PI-i/letters.length*Math.PI*2-t*.075;letter.position.set(Math.cos(a)*2.9,-Math.sin(a)*.43+.02,Math.sin(a)*2.60);letter.quaternion.copy(fixedFacing);const opacity=(.08+.92*THREE.MathUtils.smoothstep(Math.sin(a),-.15,.60))*(1-scatter*.6);letter.material.forEach(m=>m.opacity=opacity)});
 }
 ...
 window.missionHeroReady=true;host.classList.add('loaded');window.dispatchEvent(new Event('hero-ready'));requestAnimationFrame(frame);
 canvas.addEventListener('webglcontextlost',()=>{host.classList.remove('loaded');window.missionHeroReady=false;window.dispatchEvent(new Event('hero-ready'))});
}
