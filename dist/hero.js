...
async function startHero(){
 ...
 function frame(now){requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.05);last=now;if(!visible||document.hidden)return;const paused=reduced();if(!paused)t+=dt;const progress=clamp(scrollY/(document.querySelector('.hero').offsetHeight*.9));scatter+=(paused?0:progress-scatter)*Math.min(1,dt*5);if(paused)scatter=0;material.uniforms.uTime.value=t;material.uniforms.uScatter.value=scatter;globeHolder.rotation.y=t*.10;root.rotation.y+=((paused?0:mx)-root.rotation.y)*dt*3;root.rotation.x+=((paused?0:my)-root.rotation.x)*dt*3;crossHolder.rotation.y=.06;crossHolder.position.y=0;globeVeil.material.opacity=.07*(1-scatter);ring.material.opacity=1-scatter*.5;letters.forEach((s,i)=>{const a=Math.PI-i/letters.length*Math.PI*2-t*.08;s.position.set(Math.cos(a)*2.78,Math.sin(a)*-.38+.02,Math.sin(a)*2.52);s.material.opacity=(.16+.84*clamp((Math.sin(a)+.25)/1.25))*(1-scatter*.8)});renderer.render(scene,camera)}
