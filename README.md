...
The 3D modules, model, geography, and typeface are preloaded together. The first emblem shown during normal loading is the complete animated scene, with no static placeholder or crossfade. A static emblem is reserved for WebGL failure or disabled JavaScript. MISSION, WORLD, and CHURCH each orbit as one beveled, bold 3D word with natural letter spacing. Their unrotated scene group keeps the words upright and facing forward; rear words fade out to prevent overlapping ghost text. Camera framing includes each full word on narrow screens. Blue and green photo-hover particles stay within a soft cursor-following area.
